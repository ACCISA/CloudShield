...
describe('Sidebar', () => {
  describe('Full Mode', () => {
    ...
    it('calls onToggleCollapse when collapse button is clicked', () => {
      const mockToggle = jest.fn();
      renderWithRouter(<Sidebar onToggleCollapse={mockToggle} collapsed={false} />);
  ...
  describe('Collapsed State', () => {
    ...
    it('shows expand button when collapsed', () => {
      const mockToggle = jest.fn();
      renderWithRouter(<Sidebar collapsed={true} onToggleCollapse={mockToggle} />);
  ...
  describe('Provisioning Mode', () => {
    ...
    it('shows collapse button in provisioning mode', () => {
      const mockToggle = jest.fn();
      renderWithRouter(<Sidebar mode="provisioning" onToggleCollapse={mockToggle} />);
      const collapseButtons = screen.getAllByLabelText(/collapse|expand/i);
      expect(collapseButtons.length).toBeGreaterThan(0);
    });
  });
  ...
  describe('Accordion Sections', () => {
    it('expands workstations section when clicked', () => {
      ...
      const expandButton = workstationsItem.parentElement.querySelector('[aria-label*="Expand"]');
      if (expandButton) {
