...
export default function BasicInfoTab({ userData, onSave }) {
  ...
  // Only update local state for fields that are actually present.
  // This prevents the form from being wiped if the parent briefly sends
  // a partial user object after save.
  useEffect(() => {
    if (!userData) return;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (profileImage !== (userData?.profile_image ?? null)) {
  ...
  return (
    <Box>
      <Typography
        sx={{ color: themeColors.text, fontWeight: 700, fontSize: "1.1rem", mb: 0.5 }}
      >
        Basic Info
      </Typography>
      ...
      <Box sx={{ display: "flex", alignItems: "flex-start", mb: 4, gap: 4 }}>
        ...
        >
          <Typography
            sx={{ color: themeColors.text, fontWeight: 600, fontSize: "0.9rem", mb: 1 }}
          >
            Profile picture
          </Typography>
