...
def test_main_resolves_org_id_from_org_data_when_org_id_missing(monkeypatch, tmp_path):
    calls = []

    # Avoid real filesystem template copying / terraform
    def fake_copy(org_id: str, templates_dir=None, generated_dir=None):
        calls.append(("copy", org_id))
        d = tmp_path / "tfdir"
    ...
    monkeypatch.setattr(
        terraform_main,
        "run_terraform_two_phase_apply",
        lambda org_id, region, terraform_dir, count: calls.append(
            ("run", org_id, region, terraform_dir, count)
        ),
    )
    monkeypatch.setattr(
        terraform_main,
        "get_ec2_ips",
        lambda region, org_id: calls.append(("ips", region, org_id)) or ["meta"],
    )
    ...
    result = terraform_main.provision_network_terraform(
        ...
        org_data={"org_id": "ACME"},
    )

    assert result == ["meta"]
    assert ("copy", "ACME") in calls
    assert any(op[0] == "run" and op[1] == "ACME" for op in calls)
    assert any(op[0] == "ips" and op[2] == "ACME" for op in calls)


def test_main_raises_value_error_when_org_id_cannot_be_resolved(tmp_path):
    with pytest.raises(ValueError) as exc:
        terraform_main.provision_network_terraform(
            org_id=None,
            region="us-west-2",
            templates_dir=str(tmp_path / "templates"),
            generated_dir=str(tmp_path / "generated"),
    ...
    assert "requires a valid org_id" in str(exc.value)


def test_main_reuses_existing_directory_when_copy_returns_none(monkeypatch, tmp_path, caplog):
    # Make candidate_dir exist so the "reuse" branch is taken
    existing_dir = tmp_path / "generated"
    existing_dir.mkdir(parents=True, exist_ok=True)

    # Force copy_and_replace_templates -> None to simulate "already exists"
    monkeypatch.setattr(terraform_main, "copy_and_replace_templates", lambda *a, **k: None)
    ...
    result = terraform_main.provision_network_terraform(
        ...
        region="ca-central-1",
        templates_dir=str(tmp_path / "templates"),
        generated_dir=str(existing_dir),
        count=1,
        server_logger=logging.getLogger("test"),
    )

    assert result == ["meta"]
    # It should reuse candidate_dir (= abs path of generated_dir)
    assert any(op[0] == "run" and op[3] == str(existing_dir.resolve()) for op in calls)

    # This covers the warning line:
    assert any(
        "Target dir already exists" in rec.message and "Reusing existing directory" in rec.message
