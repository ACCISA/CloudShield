...
def test_cli_invokes_serve_with_parsed_args(tmp_path, monkeypatch):
    fake_server_holder = {}

    def fake_grpc_server(_executor):
        server = FakeServer()
        fake_server_holder["server"] = server
        return server

    monkeypatch.setattr(mock_grpc_server.grpc, "server", fake_grpc_server, raising=False)
    monkeypatch.setattr(mock_grpc_server, "ThreadPoolExecutor", lambda max_workers: DummyExecutor())
    monkeypatch.setattr(
        mock_grpc_server.agent_pb2_grpc,
        "add_AgentServiceServicer_to_server",
    ...
    sys_argv = [
        ...
        "1",
        "--out-file",
        str(tmp_path / "cli_requests.jsonl"),
    ]

    monkeypatch.setattr(mock_grpc_server.sys, "argv", sys_argv)

    runpy.run_module("cloudshield.Agent.tools.mock_grpc_server", run_name="__main__")

    fake_server = fake_server_holder["server"]
    assert fake_server.started is True
    assert fake_server.stopped is True
    assert fake_server.bound == ["0.0.0.0:60000"]
