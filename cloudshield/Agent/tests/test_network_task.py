...
# ensure proto + messages exist before importing the task module
...
from cloudshield.Agent.tasks.network import NetworkListingTask as _NLT
import cloudshield.Agent.tasks.network as network_mod


class DummyLogger:
    def __init__(self):
        self.messages = []
    def info(self, m, *a): self.messages.append(("info", m % a if a else m))
    def warning(self, m, *a): self.messages.append(("warning", m % a if a else m))
    def debug(self, m, *a): self.messages.append(("debug", m % a if a else m))
...
def _patch_psutil(monkeypatch, connections, name_map=None, raise_exc=None, counter=None):
    def net_connections(kind="inet"):
        ...
        return [SimpleNamespace(laddr=l, raddr=r, status=s, pid=p) for (l, r, s, p) in connections]
    class _Proc:
...
def test_get_connections_builds_entries(monkeypatch):
    l1 = SimpleNamespace(ip="127.0.0.1", port=6379); r1 = SimpleNamespace(ip="", port=0)
    l2 = ("::ffff:127.0.0.1", 50051); r2 = ("::ffff:127.0.0.1", 37760)
    _patch_psutil(monkeypatch, [(l1, r1, "LISTEN", 123), (l2, r2, "ESTABLISHED", 999)], name_map={123: "redis", 999: "python"})
    task = _NLT({"agent_id": "a"})
    conns = task.get_connections()
    assert len(conns) == 2
    c0, c1 = conns
    assert (c0.laddr_ip, c0.laddr_port, c0.raddr_ip, c0.raddr_port, c0.status, c0.pid, c0.process_name) == ("127.0.0.1", 6379, "", 0, "LISTEN", 123, "redis")
    assert (c1.laddr_ip, c1.laddr_port, c1.raddr_ip, c1.raddr_port, c1.status, c1.pid, c1.process_name) == ("::ffff:127.0.0.1", 50051, "::ffff:127.0.0.1", 37760, "ESTABLISHED", 999, "python")


def test_missing_raddr_normalized(monkeypatch):
    l = SimpleNamespace(ip="0.0.0.0", port=8000)
    _patch_psutil(monkeypatch, [(l, None, "LISTEN", 42)], name_map={42: "svc"})
    task = _NLT({"agent_id": "a"})
    conns = task.get_connections()
    assert len(conns) == 1
    c = conns[0]
    assert (c.laddr_ip, c.laddr_port, c.raddr_ip, c.raddr_port, c.process_name) == ("0.0.0.0", 8000, "", 0, "svc")
...
def test_name_cache_used_once_per_pid(monkeypatch):
    counter = {}
    l = SimpleNamespace(ip="127.0.0.1", port=1); r = SimpleNamespace(ip="", port=0)
    _patch_psutil(monkeypatch, [(l, r, "LISTEN", 7), (l, r, "ESTABLISHED", 7), (l, r, "LISTEN", 8)], name_map={7: "p7", 8: "p8"}, counter=counter)
    task = _NLT({"agent_id": "a"})
...
def test_run_sends_batch(monkeypatch):
    l1 = SimpleNamespace(ip="127.0.0.1", port=6379); r1 = SimpleNamespace(ip="", port=0)
    _patch_psutil(monkeypatch, [(l1, r1, "LISTEN", 1)], name_map={1: "svc"})
